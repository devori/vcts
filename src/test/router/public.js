...
describe('router/public.js', function () {
    ...
    before(() => {
        ...
        sinon.stub(marketApi.load('poloniex'), 'getTickers').resolves({
            USDT: {
                BTC: {
                    base: 'USDT',
                    vcType: 'BTC',
        ...
        sinon.stub(marketApi.load('binance'), 'getExchangeInfo').resolves({
            BTC: {
                ETH: {
                    rate: {
                        step: 1
    ...
    after(() => {
        ...
        marketApi.load('binance').getExchangeInfo.restore();
    });
    ...
    it('should return exchangeInfo when /markets/binance/exchange-info', done => {
        supertest(app)
            .get('/markets/binance/exchange-info')
            .expect('Content-Type', 'application/json; charset=utf-8')
            .expect(200)
            .end((err, res) => {
                if (err) {
                ...
                expect(res.body.BTC.ETH.rate.step).to.equal(1);
                expect(res.body.BTC.ETH.units.step).to.equal(2);
                done();
            });
    });
});
