...
describe('market-api/binance-api.js', function () {
    ...
    describe('getExchangeInfo', () => {
        ...
        it('should return refined exchange info when it calls', done => {
            const prom = binanceApi.getExchangeInfo();
            expect(prom).to.be.a('promise');
            prom.then((result) => {
                expect(result.BTC.ETH.rate.step).to.equal(0.000001);
                expect(result.BTC.ETH.units.step).to.equal(0.001);
                expect(result.BTC.LTC.rate.step).to.equal(0.000001);
                expect(result.BTC.LTC.units.step).to.equal(0.01);
                done();
            });
        });
    });
